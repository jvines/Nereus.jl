...
# GP priors dropped — gp_rv is not in noise_models for this run, so
# these parameter names don't exist in the Params layout. Nereus's
# strict prior validation rejects unknown names.
...
params = Params(;
    ...
    priors          = priors,
    noise_models    = [ar, ma, act],     # GP-Rot DROPPED for this run
    transdim_noise  = true,
    trend_order     = 1,
...
td = TransDimConfig(;
    ...
    noise             = true,
    toggleable        = [ar, ma, act],     # GP-Rot dropped — companion run
    # BIS regression and MA(1) both absorb short-timescale activity-driven
    # variance — BIS via direct indicator regression, MA via lag-1
    # residual correlation. They are redundant on data dominated by
    # stellar rotation modulation, and letting them compose double-counts
    # the activity signal (HD 18599 Run 6: MA+Activity won at 96.8% and
    # dragged K to 6.4 m/s, exactly the leak BIS↔GP showed in Run 4).
    # AR(1) captures longer-lag autocorrelation (different physics) and
    # remains composable with everything.
    noise_exclusion_groups = [[act, ma]],
)
