...
@testset "runner dispatch — trans-dim samplers (td block + JSON kwargs)" begin
    ...
    @testset "daedalus — Int dlogz/init_scale" begin
        ...
        @test res.chains isa MCMCChains.Chains
        @test :n_planets in names(res.chains, :parameters)
        # daedalus is a nested sampler — its real log Z must be surfaced,
...
# ---------------------------------------------------------------------------
# Registry consistency. There were once three lists (ENGINES, _KNOWN_SAMPLERS,
# _SAMPLER_FNS) and every divergence between them was a silent bug: a sampler
# reachable from run_job but not fit_*, or validated but missing a function so
# its kwargs were never checked. They are derived from one another now; this
# is the test that keeps them that way.
# ---------------------------------------------------------------------------
@testset "sampler registries agree" begin
    E = Set(keys(Nereus.ENGINES))
    K = Set(Nereus._KNOWN_SAMPLERS)
    F = Set(keys(Nereus._SAMPLER_FNS))
    U = Nereus._RUNNER_UNSUPPORTED

    # one name -> function map, not a hand-kept copy
    @test Nereus._SAMPLER_FNS === Nereus.ENGINES

    # run_job accepts exactly ENGINES minus the documented exclusions
    @test E == union(K, U)
    @test isempty(intersect(K, U))

    # nothing validated without a function behind it: that combination
    # silently skipped _assert_supported_kwargs
    @test isempty(setdiff(K, F))

    # every exclusion must be a real engine, or it is a typo that excludes
    # nothing and lets the name through
    @test issubset(U, E)

    # every registered engine is callable and declares kwargs
    for (name, fn) in Nereus.ENGINES
        @test fn isa Function
        @test Base.kwarg_decl(first(methods(fn))) isa Vector
    end

    # the exclusion is loud, not a generic "unknown sampler"
    for n in U
        @test !(n in K)
    end
end
