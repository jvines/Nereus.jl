...
# FF' extension: ActivityDecorrelation with derivative=true adds a
# `Cdot_<ind>_<inst>` coefficient on the central-difference derivative
# of the indicator. Captures rotation-phase-shifted activity-RV
# correlation that the instantaneous C alone misses (Aigrain, Pont &
# Zucker 2012, MNRAS 419, 3147).
...
for (i, name) in enumerate(inst_names)
    ...
    # FF' derivative coefficient — same range as C since the indicator
    # derivative has been computed from the EMPEROR-normalized BIS time
    # series (per-instrument mean-subtracted, scaled to mean-subtracted
    # RV peak) divided by Δt (days). Dimensionful but typically order-1
    # for a star with rotation period ~8 days and BIS swings ~10 m/s.
    priors["Cdot_bisector_span_$name"] = UniformPrior(-3.0, 3.0)
end
...
for name in inst_names
    ...
    sym_d = Symbol("Cdot_bisector_span_$name")
    if sym_d in names(chains, :parameters)
        cd = vec(Array(chains[:, sym_d, :]))
        cd_med = median(cd); cd_std = std(cd)
        sig = "$(round(abs(cd_med / cd_std), digits=1))σ"
        @printf("  Cdot_bisector_span_%-12s = %+.4f ± %.4f   (%s from zero)\n",
                 name, cd_med, cd_std, sig)
    end
end
