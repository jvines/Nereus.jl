...
const OUT_DIR   = joinpath(REPO_ROOT, "Nereus.jl", "results", "Kepler88_TTV_fast")
mkpath(OUT_DIR)
...
# --- Sample (high-D tuning) -------------------------------------------
# Goodman-Weare stretch theory: in D≫1, optimal acceptance requires
# stretch_a ≈ 1 + c/D with c~10 → stretch_a ≈ 1.07 for D=141. Use 1.1
# for safety. Pathfinder warm-start initializes walkers near the MAP,
# eliminating the long burnin from prior-distributed init.
const SEED      = parse(Int, get(ENV, "KEPLER88_TTV_SEED",    "42"))
const N_STEPS   = parse(Int, get(ENV, "KEPLER88_TTV_STEPS",   "1500"))
const N_WALKERS = parse(Int, get(ENV, "KEPLER88_TTV_WALKERS", "320"))
const N_TEMPS   = parse(Int, get(ENV, "KEPLER88_TTV_TEMPS",   "4"))
const N_BURNIN  = parse(Int, get(ENV, "KEPLER88_TTV_BURNIN",  "500"))
const STRETCH_A = parse(Float64, get(ENV, "KEPLER88_TTV_STRETCH_A", "1.1"))

println("\nsample_ptemcee (FAST): $N_TEMPS temps × $N_WALKERS walkers × $N_STEPS steps")
println("  stretch_a=$STRETCH_A (Goodman-Weare high-D tuning), init=:pathfinder")
t0 = time()
...
save_chains(joinpath(OUT_DIR, "kepler88_ttv_fast_chains.nc"), chains, params; data = data)
...
scatter!(ax, xs, y_min;
          ...
          strokewidth = 0)
save(joinpath(OUT_DIR, "oc_diagram_kepler88_fast.png"), fig)
println("\nWrote $(OUT_DIR)/oc_diagram_kepler88_fast.png")
