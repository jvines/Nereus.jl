...
"""
    as_jitter(theta, ins_idx) -> Real

Additive astrometric jitter (mas) for relative-astrometry imager
`ins_idx`, added in quadrature to that imager's reported uncertainties.

Returns exactly `0` when no astrometric instruments were configured, which
is the default: astrometry then uses the reported errors verbatim and the
likelihood is unchanged, down to the last bit (no `sqrt(sigma^2 + 0^2)`
round-trip).
"""
@inline function as_jitter(theta::Theta{T}, ins_idx::Int) where {T<:Real}
    slots = theta.params.layout.systemic.as_jitter
    (ins_idx < 1 || ins_idx > length(slots)) && return zero(T)
    slot = slots[ins_idx]
    slot == 0 && return zero(T)
    return theta.values[slot]
end
