...
function default_priors(config::ParamsConfig, data::Data)
    ...
    # --- Relative-astrometry instrumental (sharing-aware) ---
    # Additive jitter in mas, per imager. Scaled off the REPORTED errors,
    # because "mas" means something different for a speckle camera and for
    # an extreme-AO integral-field spectrograph, and a fixed prior in mas
    # would be informative for one and vacuous for the other.
    #
    # ModJeffreys with the knee at the median reported error: flat below it
    # (the region where "the quoted errors are already right" lives, and it
    # gets real prior mass, including zero), Jeffreys above (scale-free for
    # a genuinely underestimated budget). The cap at 10x the median error
    # stops the jitter from eating a companion signal wholesale.
    # Driven by the NAME LIST, not by the data: `_build_layout` allocates a
    # slot for every configured imager unconditionally, so a prior must exist
    # for every one of them or the layout has an unpriored parameter. The
    # reported errors only set the SCALE, and fall back to 1 mas when there
    # are none to look at.
    for (label, idxs) in _instrument_groups(instruments.as_names,
                                             get(sh, :as_jitter, nothing))
        errs = Float64[]
        if data.relastrom !== nothing
            relast = data.relastrom
            for idx in idxs
                mask = relast.inst .== idx
                append!(errs, relast.ra_err[mask])
                append!(errs, relast.dec_err[mask])
            end
        end
        knee = isempty(errs) ? 1.0 : max(median(errs), 1e-3)
        dic["sigma_as_$label"] = ModJeffreysPrior(knee, 10 * knee)
    end
