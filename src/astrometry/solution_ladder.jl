...
function astrom_design(iad::IADData, order::Int)
    ...
    # The ladder asks a per-CATALOGUE question — "is this source a 5p, 7p or
    # 9p astrometric solution?" — and Hipparcos and Gaia answer it separately.
    # A joint rung across a 25-yr gap is not a defined object: the
    # acceleration prior in `default_ladder_prior` is derived from ONE
    # mission's baseline, and columns 1-2 here are a single shared position
    # zero point. Handed two missions this would silently fit one frame to
    # both, and since the product is Bayes factors, an unmodelled frame
    # offset reads as curvature and pushes the posterior onto 7p/9p.
    n_iad_inst(iad) == 1 || throw(ArgumentError(
        "astrom_design: the solution ladder is single-instrument, but this " *
        "IADData carries $(n_iad_inst(iad)) instruments. Run it per mission: " *
        "`astrom_logZ(iad_for_inst(iad, m), order)`."))
    n = n_iad(iad)
