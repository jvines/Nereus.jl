...
"""
    ...
    InstrumentConfig(; rv=String[], pm=String[], as=String[])

Names of RV, photometric and astrometric instruments. Instrument indices
throughout the code refer to positions in these vectors (1-based). The
names are used only for building parameter names like `gamma_HARPS` — the
physics operates purely on integer indices.

`as` names the RELATIVE-astrometry imagers, indexed by
`RelAstromData.inst`. Each gets a `sigma_as_<name>` jitter added in
quadrature to that imager's reported uncertainties. Leave it empty (the
default) and no astrometric jitter slot is allocated at all, so an
existing relative-astrometry fit is untouched.

Two imagers of the same companion are two `as` names and ONE planet index:
GPI and SPHERE have different astrometric error budgets, and a shared
jitter lets whichever is better absorb the other's systematics.
"""
struct InstrumentConfig
    rv_names::Vector{String}
    pm_names::Vector{String}
    as_names::Vector{String}
end

InstrumentConfig(; rv=String[], pm=String[], as=String[]) =
    InstrumentConfig(rv, pm, as)
# Two-argument positional form kept for callers predating astrometric
# instruments (e.g. `InstrumentConfig(String[], ["TESS"])`).
InstrumentConfig(rv_names, pm_names) = InstrumentConfig(rv_names, pm_names, String[])
...
"""
    n_as_instruments(ic::InstrumentConfig) -> Int

Number of distinct relative-astrometry imagers -- the count of
per-instrument jitter slots the layout allocates.
"""
n_as_instruments(ic::InstrumentConfig) = length(ic.as_names)
...
- `as_jitter::Vector{Int}` : one entry per astrometric (relative-astrometry)
                             instrument; empty when none are configured
...
struct SystemicIndices
    ...
    as_jitter::Vector{Int} # one per relative-astrometry imager. Additive jitter
                            # in mas, sigma^2 -> sigma^2 + s_m^2. EMPTY unless the
                            # user names astrometric instruments, so astrometry
                            # that does not ask for it keeps using the reported
                            # uncertainties verbatim — and keeps its old numbers.
...
function _build_layout(config::ParamsConfig; data::Union{Data, Nothing}=nothing)
    ...
    # Relative-astrometry imagers. Allocated BEFORE the photometry block so
    # the parameter order stays grouped by data kind; empty by default, in
    # which case not a single name is pushed and every existing layout is
    # byte-for-byte what it was.
    as_names = config.instruments.as_names
    n_as = length(as_names)
    as_jitter = zeros(Int, n_as)
    if n_as > 0 && data !== nothing && data.relastrom !== nothing
        n_data_as = maximum(data.relastrom.inst)
        n_data_as <= n_as || throw(ArgumentError(
            "relative astrometry references imager $n_data_as but only " *
            "$n_as astrometric instrument name(s) are configured " *
            "($(as_names)). Those epochs would silently get no jitter."))
    end
    if n_as > 0
        as_groups = _instrument_groups(as_names, get(sh, :as_jitter, nothing))
        for (label, idxs) in as_groups
            push!(names, "sigma_as_$label")
            slot = length(names)
            for idx in idxs
                as_jitter[idx] = slot
            end
