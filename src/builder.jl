...
- `iad` / `relAST` — one astrometry object, or a VECTOR of them for a
  multi-instrument fit (Hipparcos IAD + Gaia DR4 epoch astrometry; two
  imagers). A vector is merged with [`merge_iad`](@ref) /
  [`merge_relast`](@ref), numbering instruments in the order given.
- `as_names` — names for the relative-astrometry imagers, in the same
  order. Supplying them allocates a `sigma_as_<name>` jitter per imager;
  leaving it empty (the default) allocates none, and the reported
  uncertainties are used verbatim as before.
...
function build_target(;
    ...
    data = Data(;
        ...
        relastrom = relAST isa AbstractVector ? merge_relast(collect(RelAstromData, relAST)) : relAST,
        hgca      = hgca,
        iad       = iad isa AbstractVector ? merge_iad(collect(IADData, iad)) : iad,
        gost      = gost,
        g23h      = g23h,
        gaia_dr3  = gaia_dr3,
    )
