...
# Sampler names run_job accepts. DERIVED from ENGINES so the two cannot
# drift. They had: ENGINES carried 17 names, _KNOWN_SAMPLERS 15 and
# _SAMPLER_FNS 14, so `ofti` was unreachable from fit_*, `ensemble`/`ess`/`map`
# were unreachable from run_job, and `pt_hmc` passed validation but was
# missing from the function map, silently skipping its kwarg check.
#
# `map` is the one deliberate exclusion: it is a point estimator that returns
# a MAPResult with NO chains. run_job summarises a posterior (median plus
# 1/2/3-sigma), and doing that to a single point produces something that looks
...
function _validate_config(cfg::AbstractDict)
    ...
    if _has(cfg, :data)
        ...
        if _has(data_cfg, :iad)
            ...
            iad_entries = iad isa AbstractVector ? iad : [iad]
            isempty(iad_entries) && push!(errs, "`data.iad` list is empty")
            for (i, e) in enumerate(iad_entries)
                if !_has(e, :hip_id) && !_has(e, :source_id) && !_has(e, :values)
                    push!(errs, "`data.iad`" * (iad isa AbstractVector ? "[$i]" : "") *
                                " requires `hip_id`, `source_id` or `values`")
                end
            end
            names_as = [_iad_block_name(e, i) for (i, e) in enumerate(iad_entries)]
            length(unique(names_as)) == length(names_as) ||
                push!(errs, "`data.iad` entries have duplicate instrument names: " *
                            "$(names_as)")
        end
    ...
    if _has(cfg, :sampler)
        ...
        if !_has(smp, :name)
            ...
            if name in _RUNNER_UNSUPPORTED
                push!(errs,
                    "sampler `$name` is a point estimator, not a posterior " *
                    "sampler: it returns no chains, and run_job would report " *
                    "median +- 1/2/3-sigma computed from a single point. " *
                    "Use fit_* or run_engine for it.")
            else
...
function _build_data(data_cfg)
    ...
    # jitter slots; the intermediate-astrometry source names are collected
    # for reporting but carry no sampled parameters of their own (their
    # nuisances are marginalised analytically).
    inst_names_as = String[]
    inst_names_as_iad = String[]
    isempty(data_cfg) &&
    ...
    # IAD: one block, or a LIST of blocks for a multi-instrument fit
    # (Hipparcos IAD + Gaia DR4 epoch astrometry). Instruments are numbered
    # by LIST POSITION, not alphabetically as RV/photometry names are: the
    # two missions have different reference epochs and different abscissa
    # conventions, and an order nobody chose is a debugging hazard when
    # reading back the per-instrument zero points.
    if _has(data_cfg, :iad)
        iad_block = _get(data_cfg, :iad)
        kw[:iad] = iad_block isa AbstractVector ?
            merge_iad([_parse_iad_block(b) for b in iad_block]) :
            _parse_iad_block(iad_block)
        inst_names_as_iad = iad_block isa AbstractVector ?
            [_iad_block_name(b, i) for (i, b) in enumerate(iad_block)] :
            [_iad_block_name(iad_block, 1)]
    end
    ...
    if _has(data_cfg, :relastrom)
        ra_block = _get(data_cfg, :relastrom)
        if ra_block isa AbstractVector
            kw[:relastrom] = merge_relast([_parse_relastrom_block(b) for b in ra_block])
            inst_names_as = [String(_get(b, :instrument; default = "AS$i"))
                             for (i, b) in enumerate(ra_block)]
        else
            kw[:relastrom] = _parse_relastrom_block(ra_block)
            _has(ra_block, :instrument) &&
                (inst_names_as = [String(_get(ra_block, :instrument))])
        end
    end
...
# Display name for one intermediate-astrometry source. Derived from the
# source id rather than defaulted to a literal, because the id is already
# in the block and "HIP24205" reads better in a log than "AS1".
function _iad_block_name(block, i::Int)
    _has(block, :instrument) && return String(_get(block, :instrument))
    _has(block, :hip_id)     && return "HIP$(Int(_get(block, :hip_id)))"
    _has(block, :source_id)  && return "GaiaDR4_$(Int(_get(block, :source_id)))"
    return "AS$i"
end
...
function _parse_iad_block(block)
    ...
    elseif _has(block, :source_id)
        # Gaia DR4 epoch astrometry. Same container, same likelihood — the
        # DR4 per-CCD abscissa model IS the Hipparcos IAD model.
        sid  = Int(_get(block, :source_id))
        path = String(_get(block, :path; default = get(ENV, "NEREUS_GAIA_DR4_XML", "")))
        xml  = (path == "" || !isfile(path)) ? fetch_gaia_dr4_prerelease() : path
        return read_gaia_epoch_votable(xml, sid).iad
    elseif _has(block, :values)
        ...
        return IADData(
            ...
            # Defaults to a full abscissa. Say `abscissa_kind: "residual"`
            # and give `ref_params` if these are O−C against a catalogue
            # solution, otherwise they cannot be combined with another
            # instrument (merge_iad refuses rather than guess).
            ref_params     = _has(vals, :ref_params) ?
                             [Tuple(Float64.(_get(vals, :ref_params)))] : nothing,
            abscissa_kind  = Symbol(_get(vals, :abscissa_kind; default = "absolute")),
        )
    else
        error("iad block needs `hip_id` (Hipparcos auto-fetch), " *
              "`source_id` (Gaia DR4 epoch astrometry) or `values`")
    end
end
...
function _parse_relastrom_block(block)
    if _has(block, :csv)
        # `load_relastrom` was never defined — this branch was an
        # UndefVarError for every config that used it. The real loader is
        # the orvara-format reader.
        return load_orvara_relast(String(_get(block, :csv)))
    elseif _has(block, :values)
        vals = _get(block, :values)
...
# The name -> function map IS ENGINES. It used to be a second, hand-kept
# copy that had already drifted: it was missing `pt_hmc`, so a run_job
# config naming pt_hmc passed validation and then silently skipped
# _assert_supported_kwargs, pushing bad keywords deep into Julia before
# failing.
...
function _dispatch_sampler(cfg, target, data, seed::Int; menu = nothing)
    ...
    elseif name == "pt"
        ...
        #
        # td IS forwarded. It previously was not, so a `transdim` block was
        # silently ignored for name = "pt" and the run came back fixed-dim
        # with no warning -- `sample_pt` has always accepted td.
        # Return shape depends on it: 2-tuple fixed-dim, 3-tuple trans-dim.
        res = sample_pt(target; td = td, kw...)
        chains, log_z = res[1], res[2]
        n_ev = length(res) >= 3 ? res[3] : 0
    ...
    elseif name == "nested_dynamic"
        ...
        return (chains = res.chains, log_evidence = res.log_z,
                 acceptance_within = Float64[], acceptance_swap = Float64[],
                 n_evals = res.n_evals)
    elseif name == "daedalus"
        td === nothing && error("daedalus requires `transdim` block")
        # sample_daedalus returns (chains, log_Z, strategy). It's a nested
        # sampler — surface its real evidence, don't discard it as NaN.
        chains, log_z, _ = sample_daedalus(target, data; td = td, kw...)
        return (chains = chains, log_evidence = log_z,
                 acceptance_within = Float64[], acceptance_swap = Float64[],
                 n_evals = 0)
    elseif name == "nuts"
    ...
    elseif name == "ofti"
        ...
        return (chains = chains, log_evidence = NaN,
                 acceptance_within = Float64[], acceptance_swap = Float64[],
                 n_evals = 0)
    elseif name == "ensemble" || name == "ess"
        # Both return a bare MCMCChains.Chains, like ofti. Single temperature,
        # no evidence estimate.
        chains = _SAMPLER_FNS[name] === sample_ess ?
            sample_ess(target, data; kw...) : sample_ensemble(target; kw...)
        return (chains = chains, log_evidence = NaN,
                 acceptance_within = Float64[], acceptance_swap = Float64[],
                 n_evals = 0)
    elseif name == "map"
        error("`map` is not available through run_job: sample_map returns a " *
              "MAPResult, a point estimate with no chains, and run_job would " *
              "summarise it as a posterior (median + 1/2/3-sigma from one " *
              "point). Use fit_* or run_engine for MAP.")
    else
...
function _write_summary(out_dir, summary)
    ...
    # estimators (NUTS, OFTI, rjmcmc, moms, daedalus) write
