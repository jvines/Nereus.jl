...
- **The Pathfinder warm start is a `pt` kwarg, not a sampler.** There is no
  ...
  `n_pathfinder_runs` (default 16) and `n_pathfinder_draws` (default 0 =
  `max(2*n_chains, 200)`), both ignored under `"prior"`
  (`src/samplers/pt.jl`).
- **`backend` is gone from every sampler.** It conflated an implementation
  swap with the algorithm choice; `sampler.name` is now the only algorithm
  selector. The Pigeons implementation of PT survives as `_sample_pt_pigeons`
  for cross-checks only — it is not a sampler name and cannot be reached from
  a job config.
