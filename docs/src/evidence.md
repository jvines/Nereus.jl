...
- **Headline estimator differs.** The in-house ensemble samplers
  ...
  the one that suits your posterior (see
...
- **`init_strategy` does not change the evidence path.** `sample_pt`
  ...
  the TI+ headline are identical either way. (`:pathfinder` is what the
  old `pt_warm` engine did for a living, and it is now a keyword.)
