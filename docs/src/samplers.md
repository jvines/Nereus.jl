...
| `daedalus`          | `sample_daedalus`          | trans | **required** | yes (NS) |
...
In-house parallel tempering: a quadratic β-ladder of RWM/slice chains with
replica swaps, threaded over chains. Supports fixed-dim **and** trans-dim
(via a `td=`). Pathfinder warmstart is an `init_strategy`, not a separate
sampler.
...
chains, log_ev = sample_pt(target;
    ...
    init_strategy  = :prior,            # :prior | :pathfinder
    n_pathfinder_runs  = 16,            # :pathfinder only; 0 disables
    n_pathfinder_draws = 0,             # 0 → max(2·n_chains, 200)
)
```

`init_strategy`:
- `:prior` **(default)** — each replica starts from a fresh prior draw of the
  systemics. Fine for unimodal posteriors; on multimodal (HD 159062-class)
  targets a prior-init replica can settle in the nearest basin.
- `:pathfinder` — `n_pathfinder_runs` independent Pathfinder L-BFGS basins,
  one **per-run** draw per chain (`n_chains > n_runs` wraps around). Not the
  PSIS-reweighted pool: on high-dimensional trans-dim targets Pareto k > 1
  routinely collapses `pf.draws` onto whichever basin Pathfinder happened to
  favour, and one draw per basin is what multi-chain PT actually wants.
  Pathfinder is run on an **unconstrained** copy of the target and the draws
  transformed back to bounded space — fed a bounded target directly, its MVN
  draws land outside support, return `-Inf`, and the PSIS reweighting turns
  `-Inf - -Inf` into `NaN` (NOI-106823 hit this consistently from
  `runner.jl`). This is what the removed `pt_warm` engine did on its only
  genuinely distinct path.

An explicit `init` wins over `init_strategy`: supply the matrix (or a
`pathfinder_init` NamedTuple, whose `.draws` field is used) and Pathfinder is
not run.

!!! warning "Pathfinder false-convergence"
    On sharp curved astrometric posteriors the Pathfinder MVN approximation
...
Run with `julia -t auto` — PT parallelises chains over threads;
single-threaded it runs the chains serially (`sample_pt` warns).

Return shape is `(chains, log_evidence)` for fixed-dim and `(chains,
log_evidence, n_evals)` when a `td` is supplied. `run_job` wraps `pt` into the
standard result tuple, surfacing `(chains, log_evidence)`.

!!! note "Pigeons is not an engine"
    The Pigeons.jl-backed path survives as the unexported
    `_sample_pt_pigeons` (`src/samplers/pt.jl`) purely to cross-check the
    in-house PT, and it is the only thing that takes an `explorer`
    (`:slice`/`:automala`, the latter gradient-based AutoMALA). It is **not**
    in `ENGINES`, has no `sampler.name`, and cannot do trans-dim: Pigeons'
    slice-sampling `explore!()` loop is not parallelised, so it runs
    single-threaded on a `NereusTarget` (~8× slower), and on HD 159062 it was
    non-reproducible across seeds (a = 115 / 41 / 78 / 64). The `backend`
    keyword that used to select it is gone — engine is now the only algorithm
    selector.
...
### `sample_daedalus` — `name: "daedalus"` (requires `transdim`)
...
```julia
chains, log_z, strategy = sample_daedalus(target, data; td = td,
    n_live                  = 200,
    dlogz                   = 0.5,
    ...
    warm_start_points       = nothing,  # e.g. from pathfinder_warmstart_daedalus
)
```
...
`pathfinder_warmstart_daedalus` builds the `warm_start_points` (MoMS
off-locations + scales) from Pathfinder draws to seed `sample_daedalus` and
...
`pathfinder_warmstart_daedalus` is a specialised variant that emits MoMS
off-locations + scales to seed `sample_daedalus` (see above).
