...
**The engine is the only algorithm selector.** How a sampler is *started* is one
of its options, not a separate entry in that table: a Pathfinder warm start is
`pt` with `init_strategy = "pathfinder"` (`n_pathfinder_runs`,
`n_pathfinder_draws` tune it), not an engine of its own.

```julia
fit_rv(rv; engine = Dict("engine" => "pt",
...
- [Samplers](samplers.md) — what the seventeen engines are and when to use each
