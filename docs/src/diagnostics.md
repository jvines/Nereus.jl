...
The gate keys on the **engine name only**, so a `pt` run with
`init_strategy = "pathfinder"` still trips it. That combination is
Pathfinder-seeded, not prior-seeded, and the limits it produces are
subject to the same mode-concentration bias as the samplers the gate
excludes — set `output.detection_limits = false` for those runs, or
keep `init_strategy` at its `"prior"` default when the K-limit curve is
the deliverable.
